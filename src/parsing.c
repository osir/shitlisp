...
lval* lval_err(char* fmt, ...) {
    ...
    // Error message can be max 511 chars long
    v->err = malloc(512);
...
lval* lenv_get(lenv* e, lval* k) {
    ...
    // If no symbol is found, check the parent environment
    if (e->par) {
...
lval* builtin_var(lenv* e, lval* a, char* func) {
    ...
    // All elements in the first list must be symbols
...
lval* builtin_if(lenv* e, lval* a) {
    ...
    // Mark both expressions as evaluated
    lval* x;
        ...
        if (strcmp(sym->sym, "&") == 0) {
            if (f->formals->count != 1) {
                ...
                return lval_err("Function format invalid. "
                        "Symbol '&' not followed by signle symbol.");
            }
...
lval* lval_read_str(mpc_ast_t* t) {
    ...
    // Copy the string without the starting quote
